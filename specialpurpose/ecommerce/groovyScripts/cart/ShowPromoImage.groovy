...
productPromoImageMap = new HashMap<String,String>()
productPromosAll.each { productPromo ->
    promoText = productPromo.promoText
    promo = org.apache.ofbiz.product.product.ProductPromoContentWrapper.makeProductPromoContentWrapper(productPromo, request)
    String url = promo.get("ORIGINAL_IMAGE_URL", "string").toString();
    if (promoText && url &&  !"N".equals(productPromo.showToCustomer )) {
        showPromoText = true
        promoToShow++
        productPromoImageMap.put(productPromo.productPromoId,url)
...
context.productPromoImageMap = productPromoImageMap
